...
#checks for non-US prices that are not labelled with the prefix '(USD)' and adds it"
#only if the price is the same as the USD price
...
def pricer(bike, priceCell):
    ...
    for language in languagePD.keys():
        if (language == 'English' or language == 'UK'):
            print(f'{language} site skipped')
        else:
            ...
            LoadBike(bike)
            #checks if this page has the same price as the sheet before adding '(USD)'
...
pricer('Alcatraz - SEP 1', 33)

pricer('Alcatraz Frame Kit', 93)

pricer('Bolinas Ridge 2 - Jul 1', 28)
pricer('Bolinas Ridge 1', 26)
pricer('Bobcat Trail 5', 22)
pricer('Bobcat Trail 4 - JUN 1', 20)
